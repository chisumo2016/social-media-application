...
//const  editAllowed = computed(() =>user.id === authUser.id)
...
//console.log(props.comment, props.post)
const deleteAllowed = computed(() =>{
    ...
    return !props.comment && props.post.group?.role == 'admin';
       ...
       // return props.user.id == authUser.id  || props.post.user.id ===authUser.id || props.post?.group?.role == 'admin'
})

function copyToClipboard() {
    // Replace 'your-text-to-copy' with the actual text you want to copy
    const textToCopy = route('post.view', props.post.id); //absolute url route('post.view', props.post.id)
...
<template>
    <Menu as="div" class="relative inline-block text-left">
        <div>
            <MenuButton
                class="w-8 h-8 rounded-full hover:bg-black/5 transition flex items-center justify-center z-10"
            >
        ...
        >
            ...
            >
                ...
                <div class="px-1 py-1">
                    ...
                    <MenuItem v-slot="{ active }" v-if="pinAllowed">
                        <button
                            @click="$emit('pin')"
                            :class="[
                    ...
                    <!--  Edit  Button  -->
                    <MenuItem v-slot="{ active }" v-if="editAllowed">
                        <button
                            @click="$emit('edit')"
                            :class="[
                                  active ? 'bg-indigo-500 text-white' : 'text-gray-900',
